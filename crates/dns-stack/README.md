...
The classic use case is **giving one DNS suffix its own answers on a
VPN-style link**: clients keep using their normal resolver for everything,
but names under one suffix (`*.myvpn`) get answered by you. (This is often
called "split DNS" — conditional forwarding by queried name — not to be
confused with split-horizon DNS, which varies answers by the *source* of a
query; this crate's resolver only ever sees local traffic and has no notion
of source.)

```text
          ┌─────────────────────── your process ───────────────────────┐
          │                                                            │
 link  ──►│  inbound packets ─► dns-stack ─► Resolver (your code)      │
...
// tokio runtime.
DnsStack::new(ra_cfg, dns_cfg).spawn(incoming_rx, outgoing_tx, Arc::new(MyResolver));
